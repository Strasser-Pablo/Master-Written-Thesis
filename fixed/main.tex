...
Note that the fact that we have truncated the sum can be considered as a slight change of initial condition.
...
\end{tikzpicture}
\caption[.]{
Numerical solution for $t=\unit{500}{\second}$, for the following methods:
\begin{align*}
&\text{Euler}&&\text{Runge-Kutta}&\\
...
\begin{figure}
\includegraphics{fixed/solheat/heatcomp}
\caption[.]{
Comparison of the solution to the analytical solution.
The form of the marker indicate the time:
\begin{align*}
 \text{\input{fixed/solheat/marko.pgf}}\qquad t&=\unit{1}{\second}&   \text{\input{fixed/solheat/mark*.pgf}}\qquad t&=\unit{300}{\second}\\
 \text{\input{fixed/solheat/markv.pgf}}\qquad t&=\unit{100}{\second}&  \text{\input{fixed/solheat/mark+.pgf}}\qquad t&=\unit{500}{\second}
\end{align*}

The color of the marker indicate the method:
\begin{align*}
&\text{Euler}&&\text{Runge-Kutta}&\\
\intertext{\centering $h=0.002$ }
 \text{\input{fixed/solheat/markg.pgf}}\qquad dt&=\unit{0.1}{\second}&   \text{\input{fixed/solheat/markc.pgf}}\qquad dt&=\unit{0.1}{\second}\\
 \text{\input{fixed/solheat/markb.pgf}}\qquad dt&=\unit{1}{\second}&  \text{\input{fixed/solheat/markr.pgf}}\qquad dt&=\unit{1}{\second}\\
\intertext{\centering $h=0.01$ }
\text{\input{fixed/solheat/marky.pgf}}\qquad dt&=\unit{0.1}{\second}&   \text{\input{fixed/solheat/markk.pgf}}\qquad dt&=\unit{0.1}{\second}
 \end{align*}
}
\label{fix:solheat:fig:comp}
\end{figure}
...
The CFL condition equation are detailed in \cite{cfl_heat}. This can be done because the viscosity is small and the non linear terms cancel.
...
This can be proved in polar coordinates in the vorticity version of the Navier-Stokes equation (not used in this work).
...
The results are plotted in figures \ref{fix:normtheta} and \ref{fix:normr}.
