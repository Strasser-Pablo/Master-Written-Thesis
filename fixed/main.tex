...
This is plotted for $2L=0.1 m$ and $\nu=1.307e-6$ witch is the viscosity of Water for 10°c and for $n_{max}=1'000'000$ in figure \ref{fixed:solheat:fig:sol}.
...
\end{tikzpicture}
\caption[.]{
Plot for the analytical solution truncated to $n_{max}=1'000'000$ with $2L=0.1 m$ and $\nu=1.307e-6$.
...
The analytical solution show that the effect of viscosity is to smooth the speed distribution. The time needed to do that is of $500 s$
witch is a very long time with respect of the distance traveled in this time ($500 m$).
...
For $dt=2$ for the Euler method and $dt=3$ for the Runge-Kutta method we have divergence (result completely wrong).
Note that the time step used is bigger than the CFL condition that is $dt_{max}=hu_{max}=0.002$. This can be done because the viscosity is small and the none linear term cancel.
