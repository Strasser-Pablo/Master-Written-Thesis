...
We see in figure \ref{topo:singl_part:fig1} that the non split method coincides with the analytical solution for the
...
When we have big time steps like $\alpha=2$ the non splitted method behaves better. But it is important to recall the results on fixed grid,
...
\begin{tikzpicture}
	\begin{axis}[width=\textwidth,use units,x unit=\second,y unit=\meter,xlabel=$t$ Position,ylabel=$y$ Position,no markers]
		...
		\addlegendentry{None Split $\alpha=2$}
