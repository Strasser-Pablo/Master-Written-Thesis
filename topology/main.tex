...
\end{description}
The use of two topologies is needed because in a staggered grid the position of speed and pressure is not the same.
...
In the argument of $f$ we have used an extrapolation of speed witch doesn't depends of the input in boundary speed (because the $E$ matrix has zero columns where needed).
...
\begin{enumerate}
	\item A neighbor cell (cell witch $E$ matrix has component with) fills.
...
\subcaptionbox{$t_1$, because the first particle has moved the domain have changed, the speed extrapolation have changed at red point.\label{topo:mult_part_error:2}}{%
...
We have seen that the extrapolation operator need to do two things:
\begin{enumerate}
...
\begin{enumerate}
\item It's a inbound cell which has no particle in it (has layer $-1$) witch represent a source of fluid of the physical problem.
...
The important point that makes that we have existence and unicity of a solution.
...
\subsubsection{Conclusion}
In conclusion, we have an algorithm of linear complexity (only one level of nested for each) which can be possibly parallelizable.

This algorithm is specialized for boundary condition in straight plane and in corner. For the other case an extrapolation scheme is used.
...
\end{center}
\caption{These are the case used in algorithm \ref{code:AlgorithmsExtrapolateNan2}. These are case where the speed component
...
For the non splitted case the acceleration is given by \ref{topo:equ:onediff}.
...
A single particle with initial $x$ speed $v_{0}$ which go from position $y$ $y_0$ and with initial time $t_0$.
...
Particle which go below the 0 altitude are supressed.

The two run took a day to complete. 3 dimmensional run was not possible for reason of time and
computing power.
